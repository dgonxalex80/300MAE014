...
Los siguientes son ejemplos de relaciones que pueden ser modeladas a través de un modelo de regesión lineal

|Contexto     |	Variable Dependiente	            |   Variables Independientes              |
|:------------|:----------------------------------|:----------------------------------------|
|Educación	  | Puntaje en un examen final	      | Horas de estudio, Calificación en exámenes previos, Asistencia a clases|
|Salud        |	Presión arterial                  |	Edad, Peso, Nivel de actividad física |
|Economía	    | Precio de una casa	              | Área construida, Número de habitaciones, Ubicación geográfica |
|Marketing	  | Ventas de un producto	            | Precio del producto, Gastos en publicidad, precios de los susititos |
|Agricultura	| Rendimiento de un cultivo	        | Cantidad de agua, Tipo de fertilizante, Temperatura |
|Deportes     |	Velocidad de un corredor	        | Edad del corredor, Tiempo de entrenamiento, Consumo de nutrientes, Raza|
|Ciencias Ambientales|	Concentración de contaminantes en el aire |	Nivel de tráfico, Actividad industrial, Condiciones meteorológicas|

<br/><br/>

Para iniciar retomaremos el concepto de correlacion entre dos variables aleatorias estudiado en la unidad anterior, el cual mide la relación lineal existente entre dos variables cuantitativas ($\rho$). Este coeficiente toma  valores desde  -1  hasta 1 como se representa en las siguientes gráficas :
...
x=muestra[,2]+100
y=muestra[,1]+100
plot(x,y, pch=19,main="Correlación Rho=-1.0", col =c1)
grid()

muestra<-gen.corr.data(-0.90,200)
x=muestra[,2]+100
y=muestra[,1]+100
plot(x,y, pch=19,main="Correlación Rho=-0.9", col =c1)
grid()

muestra<-gen.corr.data(-0.7,200)
x=muestra[,2]+100
y=muestra[,1]+100
plot(x,y, pch=19,main="Correlación Rho=-0.70", col =c1)
grid()

muestra<-gen.corr.data(-0.50,200)
x=muestra[,2]+100
y=muestra[,1]+100
plot(x,y, pch=19,main="Correlación Rho=-0.50", col =c1)
grid()

muestra<-gen.corr.data(-0.25,200)
x=muestra[,2]+100
y=muestra[,1]+100
plot(x,y, pch=19,main="Correlación Rho=-0.25", col =c1)
grid()

muestra<-gen.corr.data(0,200)
x=muestra[,2]+100
y=muestra[,1]+100
plot(x,y, pch=19,main="Correlación Rho=0", col =c1)
grid()
...
x=muestra[,2]+100
y=muestra[,1]+100
plot(x,y, pch=19,main="Correlación Rho=0.30", col =c1)
grid()
...
x=muestra[,2]+100
y=muestra[,1]+100
plot(x,y, pch=19,main="Correlación Rho=0.60", col =c1)
grid()
...
x=muestra[,2]+100
y=muestra[,1]+100
plot(x,y, pch=19,main="Correlación Rho=0.80", col =c1)
grid()
...
pch=21, bg=c2, xlim=c(0,35), ylim=c(0,25), las=1)
grid()
```
...
```{r, echo=FALSE}
plot(x,y, xlab = "Ingresos", ylab = "Consumo",
pch=21, bg=c2, xlim=c(0,35), ylim=c(0,30), las=1)
regresion=lm(y ~ x)
abline(regresion, col=c1, lwd=3)
grid()
```
...
|:----------------------------------------------------------|
|$Ho$ : $u \sim norm$                                       |
|$Ha$ : $u \hspace{.5cm} \sim no \hspace{.2cm} norm$                                   |
...
plot(x,y, xlab = "peso inicial", ylab = "peso final", pch=21, bg=c2, las=1)
grid()
```
...
plot(y,x, main="Diagrama de dispersión",xlab="X:Peso inicial", 
     ylab="Y:Peso final", pch=19, col = c2, las=1)
grid()
...
# Estimación por MCO
		...
		summary(regresion)
		confint(regresion)
		plot(y~x, xlab = "Peso inicial", ylab = "Peso final", pch=19, las=1, col= c2)
		abline(regresion)
		grid()
```
...
# Residuos
		...
		plot(valores.ajustados, residuos, las =1, pch=19, col=c2)
		grid()
```
...
# Diagnostico de normalidad
		...
		qqline(residuos)
		plot(y,x, xlab = "peso-F", ylab = "peso-b", las=1, pch=19, col =c2)
		abline(y~x)
		grid()
```
...
par(mfrow = c(4, 2))
curve(.2*x^2, from=1, to=50, n=500, xlab="x", ylab="y",col="blue", lwd=3, main="(1a)")
curve(exp(-2*x), from=1, to=4, n=500, xlab="x", ylab="y",col="blue", lwd=3, main="(1a)")
curve(2-3*log(x), from=1, to=20, n=500, xlab="xvalue", ylab="yvalue",col="blue", lwd=3, main="(1a)")
curve(.5*x^1, from=1, to=5, n=500, xlab="x", ylab="y",col="red", lwd=2, main="(1b)")
curve(2+log(x), from=1, to=20, n=500, xlab="xvalue", ylab="yvalue",col="red", lwd=3, main="(1b)")
curve(.2*x^0.1, from=1, to=20, n=500, xlab="x", ylab="y",col="green", lwd=3, main="(1c)")
curve(exp(2*x), from=2, to=5, n=500, xlab="x", ylab="y",col="green", lwd=3, main="(1c)")
curve(2+3*log(x), from=1, to=20, n=500, xlab="xvalue", ylab="yvalue",col="green", lwd=3, main="(1c)")
```
